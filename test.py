...
def organize_dir(dir_name):
    if not os.path.isdir(dir_name):
        ...
        print(dir_name + ' directory found, moving all related outputs to this directory...')      
...
mrconvert.inputs.args = '-fslgrad dwi_PE_1.bvec dwi_PE_1.bval -json_import dwi_PE_1.json -force'
mrconvert.run()

mrconvert = mrt3.MRConvert()
mrconvert.inputs.in_file = 'dwi_PE_2.nii.gz'
mrconvert.inputs.out_file = 'dwi_PE_2.mif'
mrconvert.inputs.args = '-fslgrad dwi_PE_2.bvec dwi_PE_2.bval -json_import dwi_PE_2.json -force'
mrconvert.run() 
...
mrconvert.inputs.args = '-export_pe_eddy index.txt config.txt -export_grad_fsl bvecs bvals -force'
mrconvert.run() 
...
preproc.inputs.eddy_options = '--slm=linear --cnr_maps'     # linear second level model and replace outliers
